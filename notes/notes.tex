...
[TODO: wordsmith the first patch.]
...
\begin{quote}
  ...
  the time they were added to their respective session histories. Each
  entry has an index; the latest entry has index $-1$, and the
  subsequent entries are numbered with consecutively decreasing
  integers ($-2$, $-3$, $-4$, etc).
  ...
  histories. Each entry has an index; the earliest entry has index 1,
  and the subsequent entries are numbered with consecutively
  increasing integers (2, 3, 4, etc).
\end{quote}
The definition of ``traverse the history by a delta'' begins:
\begin{quote}
  ...
  \begin{enumerate}
  \item If the index of the current entry of the joint session history
    plus \emph{delta} is less than zero or greater than or equal to the
    number of items in the joint session history, then abort these
    steps.

  \item Let \emph{specified entry} be the entry in the joint session history
    whose index is the sum of \emph{delta} and the index of the
    current entry of the joint session history.
  \end{enumerate}
\end{quote}
We propose replacing this text with the following:
\begin{quote}
  To \textbf{traverse the history by a delta} \emph{delta}, the user
  agent must append a task to this top-level browsing context's session
  history traversal queue, the task consisting of running the following
  steps:
  \begin{enumerate}
  \item If \emph{delta} is zero, then abort these steps.

  \item If \emph{delta} is positive, let \emph{specified entry} be
    the entry in the joint session future
    whose index is \emph{delta}. If there is no such entry, then
    abort these steps.

  \item If \emph{delta} is negative, let \emph{specified entry} be
    the entry in the joint session past
    whose index is \emph{delta}. If there is no such entry, then
    abort these steps.
  \end{enumerate}
\end{quote}
